...
def swap_diagonal_zeros(self:Matrix ,row: int, inverse:Matrix):
    if self.__getelem__(row, row):
...
def division_by_pivot(self: Matrix,pivot: int,row:int,inverse: Matrix):  
...
def inverse_new_element(row: int, column:int,index:int,self:Matrix,factor:float):
    newelem = (self.__getelem__(column, index) - self.__getelem__(row, index) * factor)
    self.__setelem__(column, index, newelem)
            
def row_subtraction(self:Matrix,row:int,inverse:Matrix):
        for column in range(self.cols()):
